...
It scrapes both "Yayımlananlar" (Published) and "Yayımlanacaklar" (Future) records and stores them in a Supabase PostgreSQL database.

## Features

-   **Targeted Scraping**: Fetches data for TCMB, BDDK, HMB, TÜİK, and SPK.
-   **Link Extraction**: Captures the source URL for each data item.
-   **Database Integration**: Stores data in a structured PostgreSQL database (`tuik` schema).
